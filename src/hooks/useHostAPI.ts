...
export const getHostAPIUrl = (): string => {
	...
	return apis.find((api) => api.name === current)?.url || import.meta.env.VITE_API_URL || 'https://sub.store'
}
...
export const useHostAPI = () => {
	...
	const currentUrl = computed(() => {
		return apis.value.find((api) => api.name === currentName.value)?.url ?? defaultAPI
	})
	...
	const handleUrlQuery = async ({ errorCb }: { errorCb?: () => Promise<void> }) => {
