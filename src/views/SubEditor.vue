        ...
        >
          ...
          >
            ...
            <div class="radio-wrapper">
              <nut-radiogroup
                direction="horizontal"
                v-model="form.ignoreFailedRemoteSub"
              >
                <nut-radio
                  shape="button"
                  label="disabled"
...
const subsSelectList = computed(() => {
  return subsStore.subs.map(item => {
    return [
      ...
      item.icon ||
        (appearanceSetting.value.isDefaultIcon ? logoIcon : logoRedIcon),
      item.tag
    ]
  })
})
...
const selectedSubs = computed(() => {
  if (!Array.isArray(form.subscriptions) || form.subscriptions.length === 0)
    return `: ${t(`editorPage.subConfig.basic.subscriptions.empty`)}`
...
watch(
  [tag, form.subscriptions, subsSelectList],
  () => {
    const selected = toRaw(form.subscriptions) || []
    const group =
      subsSelectList.value
